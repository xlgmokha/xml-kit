...
RSpec.describe Xml::Kit::Decryption do
  ...
  describe '#decrypt_document' do
    ...
    context "when using the ruby-saml example" do
      subject { described_class.new(private_keys: [OpenSSL::PKey::RSA.new(private_key_pem)]) }
      let(:cipher_registry) { ::Xml::Kit::Crypto }
...
PEM
      ...
      let(:symmetric_cipher) { Xml::Kit::Crypto::SymmetricCipher.new('http://www.w3.org/2001/04/xmlenc#aes128-cbc', ['\xB2\xD4p\x89\xD8k(N+\x10\xFB\xD0\xEC&\xAC\x86'].pack("H*"), 0) }
      let(:encrypted_node) do
        document.at_xpath('/samlp:Response/saml:Assertion/saml:Subject/saml:EncryptedID/xmlenc:EncryptedData', {
      ...
      before do
        allow(cipher_registry).to receive(:cipher_for) do |algorithm, private_key|
          if algorithm == 'http://www.w3.org/2001/04/xmlenc#aes128-cbc'
            Xml::Kit::Crypto::SymmetricCipher.new(algorithm, private_key, 0)
          else
            Xml::Kit::Crypto::CIPHERS.find { |x| x.matches?(algorithm) }.new(algorithm, private_key)
          end
        end
      end
